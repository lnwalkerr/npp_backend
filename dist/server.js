...
const masterRouter_1 = require("./routers/masterRouter");
const userRouter_1 = require("./routers/userRouter");
const adminRouter_1 = require("./routers/adminRouter");
const publicRouter_1 = require("./routers/publicRouter");
class Server {
    constructor() {
        this.app = express();
    }
    ...
    connectMongodb() {
        ...
        if (!databaseUrl) {
            console.log("MongoDB: No database URL provided, running without database");
            return;
        }
        ...
        try {
            mongoose
                .connect(databaseUrl, {
                maxPoolSize: 50, // Reduced for testing
                minPoolSize: 10, // Reduced for testing
                socketTimeoutMS: 10000, // Reduced timeout
                serverSelectionTimeoutMS: 5000, // Reduced timeout
                waitQueueTimeoutMS: 5000, // Reduced timeout
            })
                .then(() => {
                console.log(`MongoDB: Connected ====> ${process.env.NODE_ENV}`);
            })
                .catch((error) => {
                console.log(`MongoDB: Connection failed, running without database: ${error.message}`);
                // Don't crash the server, just log the error
            });
        }
        catch (error) {
            console.log(`MongoDB: Connection setup failed, running without database: ${error.message}`);
        }
    }
    configureBodyParser() {
    ...
    setRoutes() {
        ...
        this.app.use("/api/master", masterRouter_1.default);
        this.app.use("/api/user", userRouter_1.default);
        this.app.use("/api/admin", adminRouter_1.default);
        this.app.use("/api/public", publicRouter_1.default);
    }
    error404Handler() {
