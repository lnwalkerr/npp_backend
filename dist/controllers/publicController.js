...
class PublicController {
     ...
     */
    static getCombinedContent(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { type = 'all', search = '', status, dateFrom, dateTo, page = 1, limit = 12, sortBy = 'mixed', sortOrder = 'desc' } = req.query;
                const pageNum = parseInt(page);
                const limitNum = parseInt(limit);
                const skip = (pageNum - 1) * limitNum;
                // Build filters
                const baseFilter = { isActive: true }; // Only active content
                // Date filters
                if (dateFrom || dateTo) {
                    const dateFilter = {};
                    if (dateFrom)
                ...
                const combinedFilter = Object.assign(Object.assign({}, baseFilter), searchFilter);
                ...
                if (type === 'all' || type === 'news') {
                    queryPromises.push(news_1.default.find(combinedFilter)
                        .populate({ path: "created_by", select: "firstName lastName" })
                        .sort(sortOptions));
                    queryTypes.push('news');
                }
                if (type === 'all' || type === 'events') {
                    queryPromises.push(event_1.default.find(combinedFilter)
                        .populate({ path: "created_by", select: "firstName lastName" })
                        .sort(sortOptions));
                    queryTypes.push('event');
                }
                if (type === 'all' || type === 'videos') {
                    queryPromises.push(video_1.default.find(combinedFilter)
                        .populate({ path: "created_by", select: "firstName lastName" })
                        .sort(sortOptions));
                    queryTypes.push('video');
                }
                if (type === 'all' || type === 'leaders') {
                    queryPromises.push(leader_1.default.find(combinedFilter)
                        .populate({ path: "created_by", select: "firstName lastName" })
                    ...
                    if (contentType === 'news') {
                        transformedData = result.map((item) => {
                            ...
                            return ({
                                ...
                                image: ((_a = item.image) === null || _a === void 0 ? void 0 : _a.url) || null,
                ...
                if (sortBy === 'title') {
                    ...
                    const groupedByType = {
                        news: allData.filter(item => item.type === 'news').sort((a, b) => sortOrder === 'asc'
                            ? new Date(a.date).getTime() - new Date(b.date).getTime()
                            : new Date(b.date).getTime() - new Date(a.date).getTime()),
                        event: allData.filter(item => item.type === 'event').sort((a, b) => sortOrder === 'asc'
                            ? new Date(a.date).getTime() - new Date(b.date).getTime()
                            : new Date(b.date).getTime() - new Date(a.date).getTime()),
                        video: allData.filter(item => item.type === 'video').sort((a, b) => sortOrder === 'asc'
                            ? new Date(a.date).getTime() - new Date(b.date).getTime()
                            : new Date(b.date).getTime() - new Date(a.date).getTime()),
                        leader: allData.filter(item => item.type === 'leader').sort((a, b) => sortOrder === 'asc'
                            ? new Date(a.date).getTime() - new Date(b.date).getTime()
                            : new Date(b.date).getTime() - new Date(a.date).getTime())
                    };
                    // Interleave content types for better user experience
                    allData = [];
                    const maxLength = Math.max(groupedByType.news.length, groupedByType.event.length, groupedByType.video.length, groupedByType.leader.length);
                    for (let i = 0; i < maxLength; i++) {
                        if (groupedByType.news[i])
                            allData.push(groupedByType.news[i]);
                        if (groupedByType.event[i])
                            allData.push(groupedByType.event[i]);
                        if (groupedByType.video[i])
                            allData.push(groupedByType.video[i]);
                        if (groupedByType.leader[i])
                            allData.push(groupedByType.leader[i]);
                    }
                }
